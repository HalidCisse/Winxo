...
using Winxo.Views.Security;
using Winxo.Views.Shared;
    ...
    internal partial class Login
    {
        ...
        private void Authenticate(string userName, string pass) {
            try {
                ...
                if(App.Winxo.Authentication.Authenticate(userName, pass)) {
                    var userKey         = Membership.GetUser()?.ProviderUserKey;
                    if(userKey!=null)
                        App.CurrentUser =App.Winxo.Authentication.GetUser((Guid)userKey);
        ...
        {
            _BUSY_INDICATOR.IsBusy=true;
            var img = await Task.FromResult(App.Winxo.Authentication.GetUserPic(_PSEUDO.Text));
